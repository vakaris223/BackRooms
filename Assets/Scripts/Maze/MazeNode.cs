...
public class MazeNode : MonoBehaviour
{
    ...
    public GameObject[] wallGet()
    {
     ...
     public void SpawnLocker(LockerPos LR)
     {
         ...
         {
             case LockerPos.PosX:
                Instantiate(locker, transform.position + new Vector3(0.33f, -0.45f, 0), Quaternion.Euler(0, -90, 0));
                break;
             case LockerPos.NegX:
                Instantiate(locker, transform.position + new Vector3(-0.33f, -0.45f, 0), Quaternion.Euler(0, 90, 0));
                break;
            case LockerPos.PosY:
                Instantiate(locker, transform.position + new Vector3(0, -0.45f, 0.33f), Quaternion.Euler(0, 180, 0));
                break;
            case LockerPos.NegY:
                Instantiate(locker, transform.position + new Vector3(0, -0.45f, -0.33f), Quaternion.Euler(0, 0, 0));
